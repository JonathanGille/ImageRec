...
from similarity_search import get_images
...
def load_and_label():
    ...
    houses = get_images(houses_folder)[0:7]
    airplanes = get_images(airplane_folder)[0:7]
...
print('\nSimilaritys vorher:')
cosine_similarity_12 = F.cosine_similarity(emb1, emb2).item()
cosine_similarity_13 = F.cosine_similarity(emb1, emb3).item()
...
label_positive = torch.tensor([0.0], device=device)  # Positives Paar
label_negative = torch.tensor([1.0], device=device)  # Negatives Paar

for epoch in range(10):  # Anzahl der Epochen
    optimizer.zero_grad()

    emb1 = generate_embedding(anchors[0])
    emb2 = generate_embedding(positives[0])
    emb3 = generate_embedding(negatives[0])

    loss_pos = criterion(emb1, emb2, label_positive)
    loss_neg = criterion(emb1, emb3, label_negative)

    loss = loss_pos + loss_neg  # Gesamtverlust berechnen
    loss.backward()
    optimizer.step()
